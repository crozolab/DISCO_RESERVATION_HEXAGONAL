...
class ComandoControladorUsuarioTest {
    ...
    @Test
    @DisplayName("validar credenciales")
    void deberiaValidarCredenciales() throws Exception {
        // arrange
        ComandoUsuario usuario = new ComandoUsuarioTestDataBuilder().build("test");
        mocMvc.perform(put("/usuarios")
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(usuario)))
                .andExpect(status().isOk())
